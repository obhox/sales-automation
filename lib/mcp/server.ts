...
export function createLinkiMcpServer(input: { origin: string; auth: AuthInfo }) {
  ...
  server.registerTool("reply_intelligence_manage", {
    title:"Reply intelligence operations",description:"Fetch new email replies from the mailbox now (sync), read a contact email thread, reclassify one reply (optionally with override_kind to force the label and lift a false-positive suppression), bulk-classify pending replies, or cancel a scheduled follow-up.",
    inputSchema:{action:z.enum(["sync","thread","reclassify","reclassify_all","cancel_followup"]),reply_id:z.string().optional(),contact_id:z.string().optional(),email_account_id:z.string().optional(),override_kind:z.enum(["positive","negative","out_of_office","unsubscribe","human_review"]).optional(),confirm:z.boolean().optional()},annotations:{openWorldHint:true},
