...
export function createLinkiMcpServer(input: { origin: string; auth: AuthInfo }) {
  ...
  server.registerTool("workflow_add_step", {
    title: "Add workflow step", description: "Add a visit, connection, LinkedIn message, Sales Navigator InMail, delay, or email step to a workflow, including delivery and tracking settings. For email steps, email_subject/email_body is Variant A (control); pass email_variants (up to 3 more) to A/B test — the runner randomly picks one candidate per send and campaign analytics reports sent/open/click per variant.",
    inputSchema: { workflow_id: z.string(), step_type: z.enum(["visit", "connect", "message", "sales_inmail", "delay", "email"]), track: z.enum(["linkedin", "email"]).optional(), template_id: z.string().optional(), template_ids: z.array(z.string()).optional(), delay_seconds: z.number().int().min(0).optional(), connect_note: z.string().optional(), message_body: z.string().optional(), email_subject: z.string().optional(), email_body: z.string().optional(), email_variants: z.array(z.object({ subject: z.string(), body: z.string() })).max(3).optional(), email_signature: z.string().nullable().optional(), email_delivery_mode: z.enum(["plain", "enhanced"]).optional(), email_track_opens: z.boolean().optional(), email_track_clicks: z.boolean().optional(), ai_enabled: z.boolean().optional(), ai_model: z.string().optional(), ai_prompt: z.string().optional(), ai_max_words: z.number().int().positive().optional(), ai_language: z.string().optional() },
    annotations: { destructiveHint: false, openWorldHint: false },
  ...
  server.registerTool("linkedin_account_manage", {
    title:"LinkedIn sender accounts",description:"Fully manage LinkedIn senders: list/get/create/update/delete, disconnect (sign out but keep the account), authenticate, complete login challenges, refresh statistics, and sync accepted connections.",
    inputSchema:{action:z.enum(["list","get","create","update","delete","disconnect","authenticate","login","stats","sync_connections"]),account_id:z.string().optional(),body:z.record(z.string(),z.unknown()).optional(),confirm:z.boolean().optional()},annotations:{openWorldHint:true},
  ...
  server.registerTool("reply_intelligence_manage", {
    title:"Reply intelligence operations",description:"Start an immediate mailbox fetch for new replies and bounces (sync — returns straight away; poll sync_status for the result), read a contact email thread, reclassify one reply (optionally with override_kind to force the label and lift a false-positive suppression), bulk-classify pending replies, or cancel a scheduled follow-up.",
    inputSchema:{action:z.enum(["sync","sync_status","thread","reclassify","reclassify_all","cancel_followup"]),reply_id:z.string().optional(),contact_id:z.string().optional(),email_account_id:z.string().optional(),override_kind:z.enum(["positive","negative","out_of_office","unsubscribe","human_review"]).optional(),confirm:z.boolean().optional()},annotations:{openWorldHint:true},
  },args=>run("reply_intelligence_manage",["thread","sync_status"].includes(args.action)?"mcp:read":"mcp:execute",args,async()=>{if(!["thread","sync_status"].includes(args.action)&&!args.confirm)throw new Error("confirm=true is required to alter reply processing");
    // 'sync' starts the IMAP sweep and returns immediately — it is minutes of work on a busy
    // workspace and awaiting it here timed the call out at -32001, losing a result that had
    // in fact been produced. Poll 'sync_status' for the outcome.
    if(args.action==="sync")return api("/api/inbox/sync",{method:"POST"});
    if(args.action==="sync_status")return api("/api/inbox/sync");if(args.action==="thread")return api("/api/inbox/thread",{query:{targetId:args.contact_id,emailAccountId:args.email_account_id}});if(args.action==="reclassify_all")return api("/api/inbox/reclassify-all",{method:"POST"});if(!args.reply_id)throw new Error("reply_id is required");return api(`/api/inbox/${enc(args.reply_id)}/${args.action==="reclassify"?"reclassify":"cancel-followup"}`,{method:"POST",body:args.action==="reclassify"?{override_kind:args.override_kind}:undefined});}));
