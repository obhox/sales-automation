...
type ImapAccount={id:string;workspace_id:string;imap_host:string|null;imap_port:number|null;username:string;password:string;imap_username:string|null;imap_password:string|null;allow_self_signed:number};
...
export async function processWarmupEngagement(limit=10){
  ...
  for(const row of rows){
    const receiver=db.prepare("SELECT id,workspace_id,imap_host,imap_port,username,password,imap_username,imap_password,allow_self_signed FROM email_accounts WHERE id=?").get(row.to_account_id) as ImapAccount|undefined;
    ...
    if(shouldReply(row.id,row.reply_rate)){
      try{await sendEmailDurably({workspaceId:receiver?.workspace_id??row.workspace_id,emailAccountId:row.to_account_id,idempotencyKey:`warmup-reply:${row.id}`,source:"warmup_reply",to:row.sender_email,subject:`Re: ${row.subject}`,body:"Thanks — received this clearly. I’ll follow up when I have the next update.",headers:{"X-Linki-Warmup-Reply-To":row.id}});db.prepare("UPDATE warmup_messages SET replied_at=datetime('now') WHERE id=?").run(row.id);}catch{}
    }
  }return rows.length;
}
