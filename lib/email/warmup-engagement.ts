...
async function engageMailbox(account:ImapAccount,warmupId:string):Promise<{found:boolean;rescued:boolean}>{
  return new Promise((resolve,reject)=>{const imap=new Imap({host:account.imap_host!,port:account.imap_port??993,tls:true,tlsOptions:{rejectUnauthorized:account.allow_self_signed!==1},user:account.imap_username??account.username,password:decryptSecret(account.imap_password)??decryptSecret(account.password)!,authTimeout:10_000,connTimeout:12_000});let done=false;
...
function flattenBoxes(boxes:Imap.ImapBoxes,prefix=""):string[]{const out:string[]=[];for(const[name,box]of Object.entries(boxes)){const full=prefix?`${prefix}${box.delimiter}${name}`:name;out.push(full);if(box.children)out.push(...flattenBoxes(box.children,full));}return out;}
