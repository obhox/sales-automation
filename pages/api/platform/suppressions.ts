...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "POST") {
    const { kind, value, reason = "manual", target_id } = req.body as { kind?: SuppressionKind; value?: string; reason?: string; target_id?: string };
    if (!kind || !["email", "domain", "linkedin", "phone"].includes(kind) || !value?.trim()) return res.status(400).json({ error: "Valid kind and value are required" });
